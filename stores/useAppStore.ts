...
export const useAppStore = defineStore("app", () => {
    ...
    async function saveApp(appData: AppData): Promise<AppData> {
        ...
        try {
            ...
            if (saved) {
                const idx = appList.value.findIndex(
                    (a) => a.appId === saved.appId,
                )
                if (idx >= 0) {
    ...
    async function deleteApp(appId: string): Promise<void> {
        ...
        try {
            ...
            error.value =
                e instanceof Error ? e.message : t("app.error.deleteFailed")
            throw e
        } finally {
            isLoading.value = false
            loaderStore.hide(loaderId)
