...
/**
 * 文字列に含まれる通貨コードを先頭から検索し、一番最初にマッチした通貨データを返す
 * @param text - 検索対象の文字列
 * @return - マッチした通貨データ、またはundefined
 */
export function getCurrencyData(text: string): CurrencyData | undefined {
    const code = text.match(/([A-Z]{3})/i)?.[0]?.toUpperCase()
 ...
 * @return - 通貨単位ごとのstep値
 */
export function getExpenseStepSize(code: string): number {
    const cd = getCurrencyData(code)
	console.log('getExpenseStepSize:', code, cd?.code, !cd?.code)
    if (!cd?.code) return 1000
    // プリセット優先
	if (['KRW'].includes(cd.code)) return 5000
    if (['JPY'].includes(cd.code)) return 1000
	if (['CNY'].includes(cd.code)) return 50
	if (['USD', 'EUR'].includes(cd.code)) return 10
    // 基本は10^(-decimal_digits) * rounding
    let step = (-10) ** cd.decimal_digits * (cd.rounding || 1)
	if (step < 1) step = 1
	// ただし、0.01未満は0.01にする
 ...
 * @return - 繰り上げたY軸最大値
 */
export function getExpenseYAxisMax(
    maxValue: number,
    code: string,
    alwaysUpper = true
): number {
    const step = getExpenseStepSize(code)
    if (maxValue === 0) return step

    // 「1000単位で繰り上げ」など、step値の倍数で上限を決める
